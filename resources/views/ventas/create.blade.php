                                    ...
                                    <tr>
                                        ...
                                        <td class="border px-2 py-1 text-center">
                                            <input type="number" step="0.01" min="0" name="detalles_venta[{{ $index }}][precio_unitario]" 
                                                class="w-full rounded border-gray-300 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200 text-center"
                                                value="{{ $detalle['precio_unitario'] ?? '' }}" placeholder="Opcional">
                                        </td>
                                        <td class="border px-2 py-1 text-center">
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function calcularSubtotal(fila) {
                ...
                if (isNaN(precio) || precio <= 0) {
                    // Si no hay precio o es 0 o menor, usar el precio del producto seleccionado
                    const productoSelect = fila.querySelector('select[name$="[producto_id]"]');
                    const productoSeleccionado = productoSelect.options[productoSelect.selectedIndex];
                    precio = parseFloat(productoSeleccionado.getAttribute('data-precio')) || 0;
                }
            ...
            // Añadir atributo data-precio a las opciones de productos (para precio por defecto)
            function inicializarPrecios() {
                @json($productos).forEach(producto => {
                    const selectElems = detallesBody.querySelectorAll('select[name$="[producto_id]"]');
                    selectElems.forEach(select => {
                        for(let option of select.options) {
                            if(parseInt(option.value) === producto.id){
            ...
            // Detectar cambios para actualizar subtotales y total
            ...
            btnAgregar.addEventListener('click', function() {
                ...
                @foreach($productos as $producto)
                    opcionesProductos += `<option value="{{ $producto->id }}" data-precio="{{ $producto->precio }}">{{ $producto->nombre }}</option>`;
                @endforeach
                ...
                nuevaFila.innerHTML = `
                    ...
                    <td class="border px-2 py-1 text-center">
                        <input type="number" step="0.01" min="0" name="detalles_venta[${numFilas}][precio_unitario]" placeholder="Opcional"
                            class="w-full rounded border-gray-300 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200 text-center">
                    </td>
                    <td class="border px-2 py-1 text-center">
